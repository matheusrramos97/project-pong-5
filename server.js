...
sockets.on("connection", (socket) => {
  ...
  if (!players["Player 1"]) {
    ...
    game.addBall()
    game.start({ player1: players["Player 1"], player2: players["Player 2"] })
    game.setPlayers({ player1: players["Player 1"], player2: players["Player 2"] })
  } else {
  ...
  socket.on('disconnect', () => {
    ...
    if ((players["Player 1"] = playerId) || (players["Player 2"] = playerId)) {
      game.removeBall(1)
    }
  })
